//雇主确认合同或者拒绝合同
function submitContractStateMsg(userId,taskId,state){
	...
	var result = function(data){
		Popups(data.msg,function(){
			if(data.code == 200){
				$(".sftyb").show(500);
				window.location.href=data.url+taskId;
			}
		});
	}
...
function upload(tag,taskId){
	var f = function(path,name){
		var url = "/page/contract/saveContractFile";
		var datas = {
			"taskId":taskId,
			"path":path,
			"fileName":name
		...
		var result = function(data){
			Popups(data.msg,function(){
				if(data.code == 200){
					var tagA = "<a href='"+path+"'>"+name+"</a><br/>";
					$(".folders").append(tagA);
				}
			});
		}
